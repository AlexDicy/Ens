...
// structs automatically implement copy() and are (de)serializable by default, which also allows to print them as JSON strings. Private and protected fields are included in the serialization.
struct Rectangle {
...
// this allows for usage like `new renderer.Renderer();`
// or
import Renderer from engine.renderer;
// allows for `new Renderer();`
...
// methods that can throw exceptions are marked with `throws`, any other method can be considered safe. The list of throwable types is computed and not part of the method signature. IDEs will infer and show on hover. If any exception is not handled and the method is now marked as throws, this should result in a compilation error explaining which exceptions were not handled and how to handle them (either with a catch block or via the throws keyword).
...
// methods that throw must be called with try as a prefix, even if catched. Catch can be added as a an additional block of a method
// finally block runs last and is optional.

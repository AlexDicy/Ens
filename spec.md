...
- To read a NUL-terminated C string that a C function hands back as a foreign handle, call `fromCString` from the `@std.ffi` module: it accepts a value of any external type and copies the bytes the handle points to into a new Ens string. Freeing the C buffer stays the caller's responsibility. A null handle yields the empty string `""`.

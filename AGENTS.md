...
- Generated files (`selfhost/frontend/src/syntax/kind.ens`, `nodes.ens`, `factory.ens`, `dump.ens`) say "Generated by syntaxgen - Do not edit".
  Change `selfhost/syntax.grammar` or the emitter in `selfhost/syntaxgen/src/` and regenerate.
- Ens packages use a workspace model: a `dependencies.txt` file marks its folder as a workspace root and maps aliases to sibling packages (`@ens.frontend=../frontend`); a package is consumed through its `src/` folder.
  This mechanism is temporary (a package-manager-style definition will replace it), so keep knowledge of the file format isolated where it already lives.

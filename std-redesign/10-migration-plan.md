...
- The floating-point bit toolkit lands in two commits with a seed cut between them, because nothing in Ens can read a double's bits and std can only call a bridge once the pinned seed knows it exists.
  ...
  The second commit, landed 2026-09-19, adds `toBits`, `fromBits` and `toCanonicalBits` to `float` and `double` over those bridges, which `@std.text.numbers` now declares itself, and rewrites `isNegativeZero` on the bits.

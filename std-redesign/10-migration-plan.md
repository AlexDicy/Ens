...
- C5: `@std.io` written from scratch: `io.ens` plus the four submodules; `print`/`eprint` rerouted through the prelude onto `io.out()`.
  The two prelude functions live in `@std.io.print` (ratified 2026-09-03), since the prelude lends every export of a listed module; the compiler stops seeding `print` as a builtin and lists `std.io.print` as implicitly imported, in the same commit as the library, which no seed has to precede because the pinned seed still provides its own `print` to every module it compiles and the library's declaration takes it over inside its own module.

...
- The diagnostics review's wording item, the last of the three and the one 09-todos.md carried, landed 2026-09-20: every text a user can read under `selfhost/` and `libs/std/src` was rendered with a representative value and read against the nineteen numbered rules of the Diagnostics section in AGENTS.md, 1,326 texts over eight folders.
  The reading found 170 that missed a rule, and 196 texts changed in the end, because the ruling that every problem a command reports in its own words carries the `ens: ` prefix and the one that gave the notices and progress lines a single shape reached texts no rule had faulted.
  A proposal never changed what a message claims, only how it says it, so rule 4 came before rule 10 wherever the plainer wording would have said something else.
  The largest families were the mismatch tail that ended without a full stop wherever its hint was empty, which one mechanism fixed by ending `expectedGot` with the stop and opening every hint helper with a space instead of its own `". "`, so `sentenceSeparator` went; the argument counts that read `argument(s)`, now `1 argument`, `2 arguments` or `1 to 3 arguments` through one `argumentCountText`; the twenty-four standalone diagnostics with no full stop; the fourteen `e.g.` that are now `for example`; and the narrative rule 10 forbids, `answers with`, `asks for`, `said nothing`, `does not know` and `nothing in the environment says` among them, each replaced by the condition it stood for.
  The twenty-three panics of `libs/std` moved onto rule 17's shape, `tried to pop from an empty list. Check isEmpty() first`, and the three index panics onto `tried to read index 5 of a list of length 1. Check the index against length() first`, which 09-todos.md now names as the shape the compiler's own bound panics are to reach.
  Every problem a command reports in its own words now carries the `ens: ` prefix the driver and the cli already used, which a `ProblemRegister` on `UnitReport` and `Loaded` keeps apart from a formatted compiler diagnostic, and the notices and progress lines took one shape: a capital first letter, no trailing full stop, and names and paths in single quotes.
  Three things the reading found untrue were fixed rather than reworded: `NotADirectory` read `a directory written above it is a file` for a path that was itself the file, `createDirectory` reported an absence through the plain reason rather than the creating one, and a create blocked by a file in the path answered Win32's absence number, which `failedToCreate` now looks past by walking the parents once on Windows alone, so both platforms say `something in the path is a file, not a directory`.
  `selfhost/driver/src/cst.ens` stopped wrapping a `FileSystemError` in a prefix of its own, and `ens override` now prints its own sentence and the reading's diagnostic on separate lines, each in its own register; both were items 09-todos.md carried.
  No pin was weakened: 59 fixture directives, 56 unit-test assertion lines over 18 files and 54 assertions in `scripts/xmake_test.lua` moved onto the new texts, ten fixtures and two unit tests were added, and `spec.md` followed on the three sentences that restate a message that changed.
  Three messages still have no pin, each one for a reason the reading recorded: `.toString()` on a `decimal` cannot be reached at all, since the type is refused before a member of it is, and the two that need an unwritable temporary folder or a machine with no cache variable set are not conditions a test stages.

...
A field default that writes a sibling field, `protected int b = (a = 5);`, passes sema and builds to `Internal: assignment target has no storage slot` (`selfhost/codegen/src/lower/assignments.ens:47`), and `(a++)` in the same position reaches the increment twin at `operators.ens:110`, while `(slots[0] = 5)` in a default builds and runs because the subscript route falls back to `defaults.siblingFieldAddress` and the identifier route in `lower` does not (2026-09-22).
Whether a default may write a sibling field is a ruling, either sema refusing it or `lower` routing a slotless identifier through `siblingFieldAddress` as the address route already does; reading an earlier sibling is already allowed, which argues for the second.

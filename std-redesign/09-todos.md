...
An override may be marked less visible than the method it overrides and stays reachable through the base; ratified 2026-09-04, the C# rule: the visibility marker on an override of a class method is mandatory and must equal the overridden method's, so an unmarked, a narrower, and a wider override are all errors, while an interface implementation keeps today's rule of a marker at least as visible as its class; when the class is less visible than the method it overrides, the required marker is the base's visibility capped at the class's reach, and the diagnostic names it.

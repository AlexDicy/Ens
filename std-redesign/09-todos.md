...
Rule 12 duplicates and follow-ons: `struct Wrap<T> implements Ordered<Wrap<T>>` with a conditional `order<T: Ordered<T>>` is refused as a conditional member and then told "Method 'order' of struct 'Wrap' implements a method declared in interface 'Ordered<Wrap<T>>'; mark it 'override'.", which contradicts the first report (struct_conformance_errors_test, "a conditional member cannot satisfy a requirement").
`class Node<T> { Node<Node<T>>? child; }` used as `Node<int>` gets the never-finishes report twice, once for `Node<int>` and once for `Node<Node<T>>`, and the first one's fix names nothing written at the use (instantiation_errors_test, "a polymorphically recursive template is reported instead of expanding forever").
`class Solid { protected abstract area() -> int; }` without `abstract` gets "Class 'Solid' must override abstract method 'area', or be declared 'abstract'." beside "Abstract method 'area' requires class 'Solid' to be declared 'abstract'.", and the first is false since `Solid` declares `area` itself (phase_errors_test, "abstract methods need abstract classes and concrete subclasses must implement them").
A `const` field assigned inside a constructor loop gets the inside-a-loop refusal and the not-on-every-path refusal, whose fixes coincide (phase_errors_test, "a 'const' field takes exactly one write on every constructor path").
An interface that redeclares a method of the interface it extends makes a class missing that method reported twice, once plain and once with ", required by 'Source', which 'Buffered' extends" (phase_errors_test, "interface extension rejects bad targets, cycles, redeclarations, and leaks").
A struct value cycle `Outer` to `Inner` to `Outer?` is reported once from each struct (phase_errors_test, "struct value cycles are reported with their path").
Test programs carrying an incidental second problem, whose extra report is pinned until the program is fixed: two `main` declarations in `app.ens` report the entry-point placement once per declaration beside the overload refusal (phase_errors_test, "function collection rejects duplicate, generic, main, and external overloads"); `Wrong.speak` is private while providing `Speaker.speak` (phase_errors_test, "interface satisfaction requires every method and the override marker"); `Vague.go` is a private abstract method (phase_errors_test, "declared throws types must be distinct exception classes"); 26 private-constructor and private-field refusals on file-private types used from their own file (bodies_test "struct literal errors are reported", switches_test three tests, statements_test two tests) disappear with the file-private member ruling of 2026-09-23 and move with it.
Texts the sweep read that break a rule, pinned as they stand: rule 13 puts the fix after a semicolon in "; mark it 'override'." (six sites), "; list each bound once.", "; remove the duplicate.", "; list each exception once.", "; overloads must differ in parameter count or types.", "Duplicate switch label 'Red'; remove the extra arm.", the shift-distance and same-integer-type "; convert ... with 'as'." texts, "Literal 3000000000 does not fit in 'int' (...); declare 'long' to hold it.", "Constant 'fixed' must be given its value where it is declared; write '= ...' after the name.", "'is Base' matches every value of this switch over 'Base'; use a 'default' arm instead.", "; 'double' reaches further.", "Lazy const 'missing' needs its value on the declaration, because nothing else ever assigns it: write 'lazy const long missing = ...'." and the inference texts "'T' was inferred as 'int' from argument 1; pass the type arguments explicitly, as in 'readBoth<T>(...)'." whose example also names a type parameter the call site does not have (rules 1 and 5), as does "Write them on the type name, as in 'Pair<A, B>.describe(...)'.".
Rule 1 leaves the thing unnamed in "An interface cannot declare fields; its body lists only method signatures.", "A struct cannot declare a destructor; destructors are only allowed on classes.", "'weak' fields are not allowed on structs.", "A 'weak' field must refer to a class, not to an interface.", "A 'weak' field must be a nullable class type, for example 'weak Foo? f'." (the file's own spelling would be `weak Watcher? direct`), "A method named 'hash' must have the signature 'hash() -> long'.", "A constructor cannot be 'override' or 'abstract'.", "Only classes can use 'extends'; structs do not support inheritance.", "Declared throws type 'Error' overlaps with another type in the list", the three destructor refusals, "Shorthand declaration ';' is only allowed on a constructor." (also rule 7), "An interface method marked 'throws' must list its exception types, for example 'throws IOError'." and its abstract sibling (the file declares no `IOError`), and the bound family "Type argument 'int' for type parameter 'T' must be 'Shape' or a subclass of it." and its "must implement interface" siblings, which never name the generic and give no fix.
Rule 2 gives no next step in "Function 'area' has no parameter named 'depth'.", "'Purple' is not a member of enum 'Color'.", "Struct 'Point' has no field named 'z'.", "Field 'x' is set more than once in this struct literal.", the "expected 'int', got 'string'" argument and field texts, the switch-shape texts ("must have a 'default' arm", "only one 'default' arm", "Cannot mix value labels with 'is' arms", "An 'is' arm needs a class switch value"), "Switch labels for an integer switch must be integer constants.", the `~`, shift-distance and `&` texts on a non-integer, "Literal 300 does not fit in 'byte' (range 0..255).", "'break' can only be used inside a loop.", "'rethrow' can only be used inside a 'catch' block.", "Cannot throw a value of type 'Widget'; only 'Error' or a subclass of it can be thrown.", the `Duo<Plain>` and `Sorted<Plain>` "does not implement 'Comparable<Plain>'" texts, "Cannot assign value of type 'double' to variable of type 'float'.", "Method 'volume' of struct 'Counting' does not match 'volume() -> int' declared in interface 'Loud'.", "Cannot override 'peek' because it is declared 'final' in 'Box<int>'.", the interface-throws widening text for 'ask', the lazy const initializer "cannot call 'risky' because it can throw 'Boom'; it must not throw." (also rule 15), "'hidden' is private to class 'Secret', so the initializer of 'Reader.copy' cannot read it." and "The default value of field 'self' cannot reference the field itself.".

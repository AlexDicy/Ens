...
Test programs carrying an incidental second problem, whose extra report is pinned until the program is fixed: two `main` declarations in `app.ens` report the entry-point placement once per declaration beside the overload refusal (phase_errors_test, "function collection rejects duplicate, generic, main, and external overloads").
Texts the sweep read that break a rule, pinned as they stand: rule 13 puts the fix after a semicolon in "Duplicate switch label 'Red'; remove the extra arm.", the shift-distance and same-integer-type "; convert ... with 'as'." texts, "Literal 3000000000 does not fit in 'int' (...); declare 'long' to hold it.", "Constant 'fixed' must be given its value where it is declared; write '= ...' after the name.", "'is Base' matches every value of this switch over 'Base'; use a 'default' arm instead.", "; 'double' reaches further." and the inference texts "'T' was inferred as 'int' from argument 1; pass the type arguments explicitly, as in 'readBoth<T>(...)'." whose example also names a type parameter the call site does not have (rules 1 and 5), as does "Write them on the type name, as in 'Pair<A, B>.describe(...)'.".
Rule 2 gives no next step in "'Purple' is not a member of enum 'Color'.", "Struct 'Point' has no field named 'z'.", "Field 'x' is set more than once in this struct literal.", the "expected 'int', got 'string'" argument and field texts, the switch-shape texts ("must have a 'default' arm", "only one 'default' arm", "Cannot mix value labels with 'is' arms", "An 'is' arm needs a class switch value"), "Switch labels for an integer switch must be integer constants.", the `~`, shift-distance and `&` texts on a non-integer, "Literal 300 does not fit in 'byte' (range 0..255).", "'break' can only be used inside a loop.", "'rethrow' can only be used inside a 'catch' block.", "Cannot throw a value of type 'Widget'; only 'Error' or a subclass of it can be thrown.", the `Duo<Plain>` and `Sorted<Plain>` "does not implement 'Comparable<Plain>'" texts and "Cannot assign value of type 'double' to variable of type 'float'.".
Also: "Use 'Widget.make(...)'; a static belongs to the type, not to an instance." gives the fix before what happened; "Literal 3.5e38f is too large for 'float'" does not say to drop the `f`; "Struct literal for 'Point' does not set the field 'y'. Add 'y: ...'" suggests writing a private field; "Duplicate 'null' label." and "Duplicate switch label 'Red'; ..." differ in shape (rule 14).
The protected-constructor refusal in `checkConstructorAccess` gives no fix (found 2026-09-23 landing the file-private member rule).
